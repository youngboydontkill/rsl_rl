...
from rsl_rl.modules import ActorCritic, Enc2ActorCritic, LatentDistillationActorCritic
...
class PPO_LD:
    ...
    policy: ActorCritic|Enc2ActorCritic|LatentDistillationActorCritic
    ...
    def __init__(
        ...
        if self.distill_loss_coef > 0.0:
            if isinstance(self.policy, LatentDistillationActorCritic):
                distill_params = self.policy.get_student_distill_params()
                self.distill_optimizer = optim.Adam(distill_params, lr=distill_lr)
            elif isinstance(self.policy, Enc2ActorCritic):
                distill_params = chain(
                    self.policy.actor_props_encoder.parameters(),
                    self.policy.actor_AME2Encoder.parameters(),
                )
                self.distill_optimizer = optim.Adam(distill_params, lr=distill_lr)
            else:
    ...
    def update(self):  # noqa: C901
        ...
        for batch in batch_iter:
            ...
            # -- For distillation (separate step; does NOT affect PPO update)
            if self.distill_optimizer:
                self.distill_optimizer.zero_grad()
                obs_distill = obs_batch[:original_batch_size]
                mse_loss = torch.nn.MSELoss()
                ...
                if isinstance(self.policy, LatentDistillationActorCritic):
                    (
                        ...
                        student_perc,
                        teacher_perc,
                    ) = self.policy.get_distill_embeddings(obs_distill)

                    prop_loss = mse_loss(student_prop, teacher_prop.detach())
                    perc_loss = mse_loss(student_perc, teacher_perc.detach())
                    distill_loss_raw = 0.5 * (prop_loss + perc_loss)
                    distill_loss = distill_loss_raw * self.distill_loss_coef
                else:
                    actor_low, actor_high = self.policy.get_actor_obs(obs_distill)
                    critic_low, critic_high = self.policy.get_critic_obs(obs_distill)
                    actor_low = self.policy.actor_obs_normalizer(actor_low)
                    critic_low = self.policy.critic_obs_normalizer(critic_low)
                    actor_props = self.policy.actor_props_encoder(actor_low)
                    critic_props = self.policy.critic_props_encoder(critic_low)
                    actor_embed, _ = self.policy.actor_AME2Encoder(actor_high, actor_props, embedding_only=True)
                    critic_embed, _ = self.policy.critic_AME2Encoder(critic_high, critic_props, embedding_only=True)
                    distill_loss_raw = mse_loss(actor_embed, critic_embed.detach())
                    distill_loss = distill_loss_raw * self.distill_loss_coef
            ...
            if mean_distill_loss is not None:
                mean_distill_loss += distill_loss_raw.item()
